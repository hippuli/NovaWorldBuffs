...
function NWB:print(msg, channel, prefix, tbcCheck)
	...
	elseif (tonumber(channel)) then
		...
		if (id == 0) then
			...
			print(NWB.chatColor .. "Type \"/dmfhelper\" to open the dmf helper frame for Sayge.");
...
--Convert seconds to a readable format.
function NWB:getTimeString(seconds, countOnly, type, space, firstOnly, long)
	local timecalc = 0;
	if (countOnly) then
		timecalc = seconds;
	else
